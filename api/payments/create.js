...
module.exports=async function handler(req,res) {
  ...
  try {
    ...
    if(keys.some(key=>!["orderId","orderNumber"].includes(key))) return res.status(400).json({error:"Permintaan pembayaran berisi field yang tidak diizinkan."});
    const orderId=String(body.orderId||"").trim(),orderNumber=String(body.orderNumber||"").trim();
    if(Boolean(orderId)===Boolean(orderNumber)||orderId&&!UUID.test(orderId)||orderNumber&&!ORDER_NUMBER.test(orderNumber)) return res.status(400).json({error:"Identitas pesanan tidak valid."});
    const response=await supabase("rpc/create_manual_payment_intent",{method:"POST",body:JSON.stringify({p_order_id:orderId||null,p_order_number:orderNumber||null})});
    const data=await response.json().catch(()=>({}));
    if(!response.ok) {
      if(data.message==="ORDER_NOT_FOUND") return res.status(404).json({error:"Pesanan tidak ditemukan."});
