...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if(!response.ok) throw new Error(data.message || "Order transaction failed");
  ...
  } catch(error) { console.error(error); return res.status(422).json({error:"Pesanan tidak dapat dibuat. Periksa stok lalu coba kembali."}); }
};
