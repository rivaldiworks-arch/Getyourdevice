...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const response=await supabase("rpc/create_storefront_order_v2",{method:"POST",body:JSON.stringify({p_customer:customer,p_items:body.items.map(item=>({product_id:item.productId,quantity:item.quantity})),p_shipping_id:body.shippingId,p_payment_method:body.payment,p_payment_token:paymentToken})});
    ...
    return res.status(201).json({orderNumber:data.order_number,createdAt:data.created_at,subtotal:Number(data.subtotal),shippingCost:Number(data.shipping_cost),total:Number(data.total ?? data.grand_total),...(body.payment!=="COD"?{paymentToken}: {})});
  } catch(error) {
