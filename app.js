...
function handleAction(action) { const actions={"show-store":showStore,"show-admin":showAdmin,"open-cart":openCart,"close-cart":()=>setModal("cartDrawer",false),checkout:startCheckout,"close-checkout":()=>setModal("checkoutModal",false),"close-product":()=>setModal("productModal",false),"open-helper":()=>setModal("helperModal",true),"close-helper":()=>setModal("helperModal",false),"close-success":()=>{setModal("successModal",false);showStore();},"scroll-products":()=>$("productsSection").scrollIntoView({behavior:"smooth"}),"reset-filter":resetFilters,"reset-product":resetProductForm}; actions[action]?.(); }
