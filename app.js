...
function checkoutFieldErrors() {
  const values={custName:$("custName").value.trim(),custPhone:$("custPhone").value.trim(),custEmail:$("custEmail").value.trim(),custAddress:$("custAddress").value.trim(),custCity:$("custCity").value.trim(),custPostal:$("custPostal").value.trim()};
  const errors={};
...
async function submitOrder(event) {
  ...
  try {
    ...
    const paymentNote=payment==="COD"?"Pesanan Anda telah diterima dan sedang menunggu konfirmasi toko.":"Instruksi pembayaran akan diberikan setelah pesanan dikonfirmasi.";
