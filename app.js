...
function renderCheckout() {
  ...
  $("checkoutSummary").innerHTML = cart.map(item => { const product = products.find(entry => entry.id === item.id); return `<div class="summary-item"><span>${escapeHTML(product.name)}<small>${item.qty} × ${money(product.price)}</small></span><strong>${money(product.price * item.qty)}</strong></div>`; }).join(""); checkoutStep=1; renderCheckoutStep(); updateCheckoutTotal();
}
...
function validateCheckoutStep() { const section=document.querySelector(`[data-checkout-step="${checkoutStep}"]`); const fields=[...section.querySelectorAll("input,textarea,select")]; const invalid=fields.find(field=>!field.checkValidity()); if(invalid){$("checkoutError").textContent=invalid.validity.valueMissing?"Lengkapi semua kolom wajib sebelum melanjutkan.":"Periksa kembali format data yang Anda masukkan.";$("checkoutError").classList.remove("hidden");invalid.reportValidity();invalid.focus();return false;}return true; }
