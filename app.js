...
const CHECKOUT_STEP_FIELDS = {
  1: ["custName", "custPhone", "custEmail"],
  2: ["custAddress", "custCity", "custPostal"]
};
...
function checkoutFieldError(id, value) {
  if(id==="custName"&&value.length<3)return "Nama lengkap minimal 3 karakter.";
  if(id==="custPhone"&&!/^\+?\d{9,15}$/.test(normalizePhone(value)))return "Masukkan nomor WhatsApp yang valid, misalnya 0812… atau +62812…";
  if(id==="custEmail"&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value))return "Masukkan alamat email yang valid.";
  if(id==="custAddress"&&value.length<10)return "Alamat lengkap minimal 10 karakter.";
  if(id==="custCity"&&value.length<2)return "Kota / Kabupaten wajib diisi.";
  if(id==="custPostal"&&!/^\d{5}$/.test(value))return "Kode pos Indonesia harus terdiri dari 5 angka.";
  return "";
}
function validateCheckoutStep(step = checkoutStep) {
  clearFieldErrors();
  const section=document.querySelector(`[data-checkout-step="${step}"]`);
  if(!section){console.error(`Checkout step ${step} tidak ditemukan.`);return false;}
  for(const id of CHECKOUT_STEP_FIELDS[step]||[]){
    const field=$(id); if(!field)continue;
    const message=checkoutFieldError(id,field.value.trim());
    if(message){const errorNode=document.querySelector(`[data-field-error="${id}"]`);if(errorNode)errorNode.textContent=message;field.setAttribute("aria-invalid","true");$("checkoutError").textContent="Periksa kembali data yang ditandai.";$("checkoutError").classList.remove("hidden");field.focus();return false;}
  }
  const nativeInvalid=[...section.querySelectorAll("input,textarea,select")].find(field=>!field.checkValidity());
  if(nativeInvalid){$("checkoutError").textContent=step===5?"Centang persetujuan setelah memeriksa pesanan.":"Lengkapi pilihan wajib sebelum melanjutkan.";$("checkoutError").classList.remove("hidden");nativeInvalid.focus();return false;}
  return true;
}
...
function handleAction(action) { const actions={"show-store":showStore,"show-orders":showCustomerOrders,"show-admin":showAdmin,"open-cart":openCart,"close-cart":()=>setModal("cartDrawer",false),checkout:startCheckout,"checkout-next":()=>changeCheckoutStep(1),"checkout-back":()=>changeCheckoutStep(-1),"close-checkout":()=>setModal("checkoutModal",false),"close-product":()=>setModal("productModal",false),"open-helper":()=>setModal("helperModal",true),"close-helper":()=>setModal("helperModal",false),"close-success":()=>{setModal("successModal",false);showStore();},"success-orders":()=>{setModal("successModal",false);showCustomerOrders();},"scroll-products":()=>$("productsSection").scrollIntoView({behavior:"smooth"}),"reset-filter":resetFilters,"reset-product":resetProductForm}; actions[action]?.(); }
