...
if (!html.includes('id="checkoutNext" class="primary" type="button" data-action="checkout-next"')) throw new Error("Checkout next button must be a non-submit action button");
for (const marker of ['1: ["custName", "custPhone", "custEmail"]', '2: ["custAddress", "custCity", "custPostal"]', "function validateCheckoutStep(step = checkoutStep)", "if(errorNode)errorNode.textContent=message"]) {
  if (!javascript.includes(marker)) throw new Error(`Step-scoped checkout validation missing: ${marker}`);
}
if (javascript.includes("checkoutFieldErrors()")) throw new Error("Checkout validation must not calculate errors for later steps");
